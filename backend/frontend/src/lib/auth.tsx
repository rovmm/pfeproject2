...
type AuthContextValue = {
  ...
  logout: () => void;
  updateUser: (patch: Partial<Pick<User, 'name' | 'initials'>>) => void;
};
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const applyAuthResponse = (res: { token: string | null; id: number; fullName: string; email: string; role: string; plan?: string }) => {
    const u: User = {
      id: res.id,
      name: res.fullName,
  ...
  const value = useMemo<AuthContextValue>(
    () => ({
      ...
      registerWithCredentials: async (fullName, email, password, role) => {
        // Backend never returns a token here — the account is unverified
        // until the OTP code is confirmed via verifyEmailCode.
        await authApi.register({ fullName, email, password, role: roleToBackend(role) });
      },
